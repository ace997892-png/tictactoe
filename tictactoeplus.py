...
def set_tile(row, col):
    ...
    if vs_bot and curr_player == playerO and not game_over:
        window.after(400, bot_move)
...
def bot_move():
    move = find_winning_move(playerO)
    if move:
        set_tile(move[0], move[1])
        return

    move = find_winning_move(playerX)
    if move:
        set_tile(move[0], move[1])
        return
...
def new_match():
    global player1_score, player2_score, player1_name, player2_name, vs_bot
    ...
    name_entry1.delete(0, tkinter.END)
    name_entry2.delete(0, tkinter.END)
    vs_bot_var .set(False)

    new_game()

def set_names():
    global player1_name, player2_name, vs_bot
    if name_entry1.get().strip() !="":
        player1_name = name_entry1.get().strip()
    if name_entry2.get().strip() !="":
        player2_name = name_entry2.get().strip()

    vs_bot = vs_bot_var.get()
    if vs_bot: 
        player2_name = "Bot"
...
intro_frame = tkinter.Frame(window, background=color_dark_grey, padx=60, pady=50,
                             highlightbackground=color_blue, highlightthickness=4)
intro_frame.pack(padx=30, pady=30)

title_label = tkinter.Label(intro_frame, text="TIC TAC TOE", font=("Consolas", 40, "bold"),
                            background=color_dark_grey, foreground=color_blue)
title_label.pack(pady=(0, 5))

icon_label = tkinter.Label(intro_frame, text="X O X", font=("Consolas", 28, "bold"),
                           background=color_dark_grey, foreground="white")
icon_label.pack(pady=(0, 15))

subtitle_label = tkinter.Label(intro_frame, text="Two players, or challenge the bot!",
                               font=("Consolas", 13), background=color_dark_grey, foreground="white")
subtitle_label.pack(pady=(0, 25))

play_button = tkinter.Button(intro_frame, text="▶ PLAY", font=("Consolas", 18, "bold"),
                             background=color_blue, foreground="white", activebackground=color_red,
                             command=lambda: _start_game())
play_button.pack()

#center the intro screen on first launch
window.update()
...
vs_bot_var = tkinter.BooleanVar()
vs_bot_check = tkinter.Checkbutton(name_frame, text="Play vs Bot (Player 2)", variable=vs_bot_var,
                                   background=color_dark_grey, foreground="white", selectcolor=color_dark_grey)
vs_bot_check.grid(row=3, column=0, columnspan=2, pady=5)
# --- END BLOCK ---
...
new_match_button = tkinter.Button(frame, text="new match", font=("Consolas", 20), background=color_dark_grey,
                                    foreground="white", command=new_match)
new_match_button.grid(row=4, column=2, columnspan=1, sticky="we")
...
def _start_game():
    ...
    #reset geometry so the window can resize to fit the new content
    window.geometry("")
